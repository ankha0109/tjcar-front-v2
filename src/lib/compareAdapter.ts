...
function koreaValues(
  ...
  return {
    ...
    bodyType: orDash(item.bodyType),
    color: orDash(item.color),
    priceOriginal: formatMoney(
      car.price_original,
      car.price_original_currency ?? "KRW",
    ),
    priceUsd: formatMoney(car.price_usd, "USD"),
    avgPrice: null,
    priceMnt:
    ...
    drive: orDash(car.drive_type),
    auctionName: null,
    auctionDate: null,
    lot: null,
    rate: null,
    equipment: car.features?.length ? car.features.join(", ") : null,
    accident: car.has_accident ?? null,
    owners: car.owner_count != null ? String(car.owner_count) : null,
    verified: car.is_verified ?? null,
    region: orDash(item.location),
    evaluation: evaluationSheet(item.images),
  };
}
