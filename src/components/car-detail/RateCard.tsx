...
type Props = {
  ...
  rate: string;
  /** Localized "RATE" label — the tile's own caption. */
  label: string;
};
 ...
 * from.
 *
 * One shell everywhere: the square tile, paired with the tugrik headline price
 * in a 2/5 – 3/5 row. `/japan/{id}` puts it beside the landed estimate and
 * `/garage/{id}` beside the asking price, so the grade reads the same on a lot
 * we are bidding on and on a car we already own. It once had a wide "bar" shell
 * for the in-stock page that spelled the symbol out ("4 · Машины үнэлгээ ·
 * Сайн"); the two pages are one layout now and it is gone.
 */
