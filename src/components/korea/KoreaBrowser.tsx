...
type Props = {
  ...
  initialFilters: KoreaFilterValues;
...
function serializeFilters(filters: KoreaFilterValues): string {
  ...
  for (const [k, v] of Object.entries(koreaFiltersToQuery(filters))) {
    params.set(k, String(v));
  }
  return params.toString();
}

export default function KoreaBrowser({
  initialPage,
  initialFilters,
  ...
  // Sync filters → URL (debounced) so the page is shareable and the server can
  // hydrate page 1 with the same filters on a fresh load.
  useEffect(() => {
  ...
  const infinite = useKoreaInfinite(filters, initialPage);
